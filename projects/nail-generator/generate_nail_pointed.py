...
# 参数（单位：mm）
head_radius = 5.0       # 钉头半径
head_height = 10.0      # 钉头高度
neck_radius = 4.0       # 环槽处半径
shank_radius = 4.0      # 钉体半径
shank_length = 3.0      # 圆柱部分长度（减短给尖头留空间）
tip_length = 4.0        # 尖头长度
...
# 钉体圆柱部分（z: -5 到 -2，高度3mm）
all_triangles.extend(create_cylinder_triangles(shank_radius, shank_length, -shank_length-2))

# 尖头圆锥部分（从z=-5到z=-9）
all_triangles.extend(create_cone_triangles(shank_radius, tip_length, -shank_length-2))

# 写入文件
write_stl('/root/.openclaw/workspace/小钉子带尖.stl', all_triangles)

print("✅ STL文件已生成：小钉子带尖.stl")
print(f"📐 规格：")
