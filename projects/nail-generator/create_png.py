...
# 绘制标题
draw.text((300, 40), '小钉子设计图', fill=color_dark, font=font_title, anchor='mm')
...
# 坐标计算（从上到下）
head_top = offset_y + 212
head_bottom = head_top + head_h * scale
neck_top = head_bottom
neck_bottom = neck_top + neck_h * scale
shank_top = neck_bottom
shank_bottom = shank_top + shank_h * scale
tip_top = shank_bottom
tip_bottom = tip_top + tip_h * scale
...
# 尖头
points = [(offset_x, tip_bottom), (shank_l, tip_top), (shank_r, tip_top)]
draw.polygon(points, fill=color_blue_light, outline=color_dark, width=2)
draw.text((shank_r + 15, tip_top + tip_h*scale//2), f'尖头 {tip_h}mm', fill=color_dark, font=font_text)
...
# 保存
img.save('/root/.openclaw/workspace/小钉子示意图.png')
print('PNG示意图已生成：小钉子示意图.png')
